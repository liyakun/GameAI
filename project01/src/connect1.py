...
class ConnectFour(object):
    ...
    def __init__(self):
        ...
        self._players[0] = _RandomPlayer(self._COLORS[0])
...
class _RandomPlayer(_Player):
    def __init__(self, color):
        super(_RandomPlayer, self).__init__(color)
        self._type = "x"

    def get_move(self, grid):
        return int(random.randint(0, CONNECT_FOUR_GRID_WIDTH-1 ) )
...
class _ComputerPlayer(_Player):
    ...
    def _get_best_move(self, grid):
        ...
        if self._color == CONNECT_FOUR_COLORS[0]:
            random_color = CONNECT_FOUR_COLORS[1]
        else:
            random_color = CONNECT_FOUR_COLORS[0]
        ...
        for col in xrange(CONNECT_FOUR_GRID_WIDTH):
            if self._is_legal_move(col, grid):
                ...
                legal_moves[col] = -self._find(self._DIFFICULTY - 1, tmp_grid, random_color)
    ...
    def _eval_game(self, depth, grid, player_color):
        ...
        # get scores of random and IA player with theirs streaks
        ...
        random_fours = self._find_streak(grid, opp_color, 4)
        random_threes = self._find_streak(grid, opp_color, 3)
        random_twos = self._find_streak(grid, opp_color, 2)
        # calculate and return the alpha
        if random_fours > 0:
            return -100000 - depth
        else:
            return (ia_fours * 100000 + ia_threes * 100 + ia_twos * 10) - (random_threes * 100 + random_twos * 10) + depth
